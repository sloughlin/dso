...
class SampleOutputWrapper : public Output3DWrapper
{
  private:
      ...
      std::map<int, Keyframe*> keyframesByKFID;
...
public:
        ...
        virtual void publishGraph(const std::map<uint64_t, Eigen::Vector2i, std::less<uint64_t>, Eigen::aligned_allocator<std::pair<const uint64_t, Eigen::Vector2i>>> &connectivity) override
        {
            ...
            int maxWrite = 5;
            connections.resize((int)connectivity.size());
            int runningID = 0;
            ...
            for(const std::pair<uint64_t,Eigen::Vector2i> &p : connectivity)
            {
                ...
                connections[runningID].from = 
                    keyframesByKFID.count(idHost) == 0 ? 0 : keyframesByKFID[idHost];
                connections[runningID].to = 
                    keyframesByKFID.count(idTarget) == 0 ? 0 : keyframesByKFID[idTarget];
                ...
                uint64_t inverseKey = (((uint64_t)idTarget) << 32) + ((uint64_t)idHost);
        ...
        virtual void publishKeyframes( std::vector<FrameHessian*> &frames, bool final, CalibHessian* HCalib) override
        {
            for(FrameHessian* f : frames)
            {
                ...
                if (keyframesByKFID.find(f->frameID) == keyframesByKFID.end()){
                    Keyframe* kf = new Keyframe();
                    keyframesByKFID[f->frameID] = kf;
                    keyframes.push_back(kf);
                }
                keyframesByKFID[f->frameID]->setFromKF(f, HCalib);
